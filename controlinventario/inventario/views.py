    ...
    if request.GET.get("codigo") is not None:
        for producto in productos:
            if request.GET.get("codigo") == producto.cod_barra and producto.estado_prod is False:
                ...
                producto.save()
                messages.success(
                    request, "La merma de " + producto.nombre + " ha sido ingresada correctamente."
                )
                return redirect("editar_merma")
        messages.error(request, "El código ingresado no está asociado a ningún producto.")
...
def MostrarEstadistica(request):
    if request.POST:
        ...
        response["Content-Disposition"] = "attachment; filename=Reporte1.xlsx"
        return response
    else:
        form = Reporte1Form()
...
def MostrarEstadistica2(request):
    if request.POST:
        ...
        response["Content-Disposition"] = "attachment; filename=Reporte2.xlsx"
        return response
    else:
        form = Reporte2Form()
